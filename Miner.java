...
public class Miner implements Runnable{
    ...
    public boolean doProofOfWork(Block oBlock) {
        ...
        String sDifficulty = ""; // sDifficulty currently empty

        while(sDifficulty.length() < oBlock.getDifficulty()) { //Creating string sDifficulty with as many zeros in it as oBlock.getDifficulty equals

            sDifficulty  = sDifficulty + "0"; //0 in quotes because sDifficult is a String

        }

        int nonce = 0; //nonce starting value of zero

        while(true) { //while loop to check if block/nonce has been mined already

            if(bAbortPoW == true) {

                bAbortPoW = false;

                System.out.println("[miner] Aborted mining block. Possible result of  confirmed block being mined elsewhere.");
                return false;

            }

            else {
                oBlock.setNonce(String.valueOf(nonce)); // on oBlock, set nonce
                oBlock.computeHash(); //compute hash


                if(oBlock.getNonce().startsWith(sDifficulty)) {
                    return true; //returns true when nonce/block is verified to not have been mined yet
                }
