...
public class Block {
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> lstItems) {
        ...
        //Generating case for 2 items size with if statement
        if(lstItems.size() == 2) {

            MerkleNode oLeafNode1 = new MerkleNode();
            MerkleNode oLeafNode2 = new MerkleNode();

            oLeafNode1.sHash = BlockchainUtil.generateHash(lstItems.get(0));
            oLeafNode2.sHash = BlockchainUtil.generateHash(lstItems.get(1));

            populateMerkleNode(oMerkleNode, oLeafNode1, oLeafNode2); //calculates the merklenode by combining leafNode1, and leafNode2)

        }

        //Generating case for 4 items size with if statement
        if(lstItems.size() == 4) {

            MerkleNode oLeafNode1 = new MerkleNode();
            MerkleNode oLeafNode2 = new MerkleNode();
            MerkleNode oLeafNode3 = new MerkleNode();
            MerkleNode oLeafNode4 = new MerkleNode();


            oLeafNode1.sHash = BlockchainUtil.generateHash(lstItems.get(0));
            oLeafNode2.sHash = BlockchainUtil.generateHash(lstItems.get(1));
            oLeafNode3.sHash = BlockchainUtil.generateHash(lstItems.get(2));
            oLeafNode4.sHash = BlockchainUtil.generateHash(lstItems.get(3));




            populateMerkleNode(oMerkleNode,oLeafNode1,oLeafNode2);
            populateMerkleNode(oMerkleNode,oLeafNode3,oLeafNode4); //calculates the merkle node by combining oLeafNodes from 1 to 4)
        }
    ...
    private void populateMerkleNode(MerkleNode oNode, MerkleNode oLeftNode, MerkleNode oRightNode){
        ...
        //setting node's hash
        oNode.sHash = BlockchainUtil.generateHash(oLeftNode.sHash + oRightNode.sHash); //similar to return in computeHash by referencing BlockchainUtil().generateHash
		oNode.oLeft = oLeftNode;
		oNode.oRight = oRightNode;  //referenced from in class review of populateMerkleNode
